...
var NPC = enchant.Class.create(enchant.Sprite,{
    initialize:function(options) {
        ...
        this.image = g.assets[options.image];
    ...
    facingSquare:function() {
        ...
        if((facingSquare.x < 0 || facingSquare.x >= g.mapList[g.mapCode].width / config.spriteWidth) ||
            (facingSquare.y < 0 || facingSquare.y >= g.mapList[g.mapCode].height / config.spriteHeight)) {
            return null;
    ...
    renewMove:function() {
        ...
        //this.duration = rangeRand(80,100);
        this.duration = rangeRand(50,80);
        this.distance = 32 * rand(4);
    },
    ...
    move:function() {
        ...
        if(this.isMoving) {
            ...
            if(this.xMovement || this.yMovement) {
                ...
                if(x >= 0 && x < g.mapList[g.mapCode][0].width &&
                    y >= 0 && y < g.mapList[g.mapCode][0].height &&
                    !g.mapList[g.mapCode][0].hitTest(x,y) &&
                    (x !== p1.player.x || y !== p1.player.y) &&
                    !this.intersect(p1.player)) {

                    if(this.checkNpcObstacle(mapCode['desertTown'])) return;
                    this.isMoving = true;
                    this.move();
                } else {
                    this.renewMove();
    ...
    checkNpcObstacle:function(mapCode) {
        var npc = g.npcList[mapCode].npc.filter(function(o) {
    ...
    getNextTile:function() {
        var curTileX = (this.x / config.spriteWidth) >> 0,
    ...
    pushMove:function() {
        if(this.stop) {
            if(this.canPush) {
                var player = g.playerList[0].player;
                ...
                switch (player.aspect) {
                    case 'right':
                        ...
                        if(g.mapList[g.mapCode][0].hitTest(this.x + 32,this.y)) {
                            this.walk=2;
                        }
                        this.x = player.x + 32;
                        break;
                    case 'up':
                        ...
                        if(g.mapList[g.mapCode][0].hitTest(this.x,this.y - 1)) {
                            this.walk = 2;
                        }
                        this.y = player.y - 32;
                        break;
                    case 'down':
                        ...
                        if(g.mapList[g.mapCode][0].hitTest(this.x,this.y + 32)) {
                            this.walk = 2;
                        }
                        this.y = player.y + 32;
                        break;
                    case 'left':
                        ...
                        if(g.mapList[g.mapCode][0].hitTest(this.x - 1,this.y)) {
                            this.walk = 2;
                        }
                        this.x = player.x - 32;
                        break;
    ...
    specialMove:function() {
        ...
        if(this.specialAction) {
            if(!this.sequence.length && !this.specialActionFinish) {
                for (var i in this.specialAction) {
                    var item = this.specialAction[i];   //{x:5}
                ...
                if(this.sequence.length) {
                    if (Object.keys(this.sequence[0])[0] === 'x') {
                        ...
                        this.frame = this.direction * 4 + this.walk;
                        var dx = this.x - this.sequence[0].x * config.spriteWidth;
                        //若到达终点
                        if (dx === 0) {
                        ...
                        var dy = this.y - this.sequence[0].y * config.spriteHeight;
                        if (dy === 0) {
                            this.sequence.shift();
