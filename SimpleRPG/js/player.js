...
Player.prototype = {
    ...
    addPlayer:function(options) {
        var player = new Sprite(config.spriteWidth,config.spriteHeight);
        ...
        player.image = new Surface(options.imgWidth || 128,options.imgHeight || 128);
        player.image.draw(g.assets[options.playerImage]);
    ...
    getNextTile:function(inputDirection) {
        var curTileX = (this.player.x / config.spriteWidth) >> 0,
    ...
    pushObject:function(player,direction,mapCode) {
        var npc = g.npcList[mapCode].npc;

        if(npc && npc.length) {
            var obstacle = hasObstacleAround(player).filter(function (o) {
        ...
        switch (direction) {
            case 'right':
                ...
                if(((npc.checkNpcObstacle(mapCode) && player.direction === npc.direction) ||
                    g.mapList[mapCode][0].hitTest(npc.x + 32,npc.y)) &&
                    player.x === (npc.x - 32) && player.y === npc.y) {
                    player.x = npc.x - 32;
            ...
            case 'up':
                if(((npc.checkNpcObstacle(mapCode) && player.direction === npc.direction) ||
                    g.mapList[mapCode][0].hitTest(npc.x,npc.y - 32)) &&
                    player.x === npc.x && player.y === (npc.y + 32)) {
                    player.y = npc.y + 32;
                    player.isMoving = false;
                    player.walk = 2;
                    return true;
    ...
    sequenceMove:function(direction) {
        var npc = g.npcList[g.mapCode].npc.filter(function(o) {return o.canPush === true})[0];
        if(!npc) return false;
    ...
    move:function() {
        ...
        if(that.isMoving) {
            ...
            if(!(g.frame % 4)) {//每4帧更新一个动作
                that.walk++;
                that.walk %= 4;//0,1,2,3
            }
            //如果完成一次移动，角色正好立于一个tile上时，停止移动并设定站立姿势
            if((that.xMovement && that.x % 32 === 0) || (that.yMovement && that.y % 32 === 0)) {
                that.isMoving = false;
                that.walk = 2;
                if(g.encounter) {
            ...
            that.speed = 4;
            if(g.npcList[g.mapCode].npc && g.npcList[g.mapCode].npc.length) {
            ...
            if(g.input.up) {//上
                that.aspect = 'up';
                that.direction = 3;
                this.pushObject(this,'up',g.mapCode);

                if(g.npcList[g.mapCode].npc.length) {
                    npc = g.npcList[g.mapCode].npc.filter(function(o){ return o.canPush === true})[0];

                    if(this.checkHitTest(that.aspect,g.mapCode,npc)) return;
                    if(this.sequenceMove(that.aspect)) return;
                }

                that.yMovement = g.dirDeny[that.aspect] ? 0 : -that.speed;    //每帧移动4像素

            } else if(g.input.right) {//右
                that.aspect = 'right';
                that.direction = 2;

                this.pushObject(this,that.aspect,g.mapCode);
                ...
                that.xMovement = g.dirDeny[that.aspect] ? 0 : that.speed;
            } else if(g.input.down) {//下
                that.aspect = 'down';
                that.direction = 0;
                this.pushObject(this,'down',g.mapCode);
                ...
                that.yMovement = g.dirDeny[that.aspect] ? 0 : that.speed;
            } else if(g.input.left) {//左
                that.aspect = 'left';
                that.direction = 1;
            ...
            if(that.xMovement || that.yMovement) {
                ...
                var x = that.x + (that.xMovement ?
                    that.xMovement / Math.abs(that.xMovement) * config.spriteWidth : 0);
                var y = that.y + (that.yMovement ?
                    that.yMovement / Math.abs(that.yMovement) * config.spriteHeight : 0);
                if(x >= 0 && x < config.mapWidth && y >= 0 && y < config.mapHeight && !this.map.hitTest(x,y)) {
                    that.isMoving = true;
                    this.move();
                }
            }
